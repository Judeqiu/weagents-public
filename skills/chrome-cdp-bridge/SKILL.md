---
name: chrome-cdp-bridge
description: Use when needing to control Chrome browser via CDP (Chrome DevTools Protocol) for web automation, scraping, or browser tasks. Provides easy connection to local or remote Chrome instances with persistent sessions. Use for any browser automation that requires real Chrome with user profiles.
---
...
```bash
~/.openclaw/workspace/skills/chrome-cdp-bridge/scripts/chrome_manager.sh start
```
